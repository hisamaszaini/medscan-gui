...
class ImageCapturePage(QWidget):
	...
	def start_camera(self, screening_type):
		...
		try:
			...
			config = self.picam.create_still_configuration(
				main={"size": (3280, 2464)},  # Full-res capture
				lores={"size": (640, 480)},  # Live preview
				display="lores"
			)
			self.picam.configure(config)
			self.picam.start()
			time.sleep(1.5)  # Stabilisasi AE/AWB
			...
			self.video_display.setText(f"Gagal memulai Picamera2: {e}")
			print(e)
	...
	def on_capture_clicked(self):
		...
		try:
			...
			self.captured_pixmap = self._convert_frame_to_pixmap(frame_resized)
