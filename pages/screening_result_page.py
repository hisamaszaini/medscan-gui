...
class ScreeningResultPage(QWidget):
    ...
    def start_analysis(self, screening_type, patient_data, image_pixmap):
        ...
        # Bersihkan worker lama jika ada
    ...
    def on_analysis_finished(self, result_data):
        try:
            status = "Normal"
            summary = "Tidak ada tanda-tanda abnormalitas terdeteksi."
            confidence = 100.0
            ...
            if detections:
                det = detections[0]
                try:
                    confidence = float(det.get('conf', 0)) * 100
                except (ValueError, TypeError):
                    confidence = 0

                status = det.get('class', 'Unknown')
            ...
            # Download gambar hasil jika ada
            ...
            if image_path:
                try:
                    ...
                    print(f"Warning: Gagal download gambar - {e}")
